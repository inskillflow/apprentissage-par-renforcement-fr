...
| 18 | [Exemple chiffré — TD(n) vs TD(λ) (achat du lundi)](#section-18) |
...
<details>
<summary>18 — Exemple chiffré : TD(n) vs TD(λ) — l'achat du lundi</summary>

<br/>

> Cette section reprend **un même scénario** et l'évalue successivement avec **TD(0)**, **TD(1)**, **TD(2)**, **TD(3)** puis **TD(λ)**, pour montrer concrètement **pourquoi** mélanger les horizons change la conclusion.

---

### 18.1 — Le scénario

Un trader achète un actif lundi. La question : **acheter lundi était-il une bonne décision ?** On observe ensuite les prix sur 4 jours.

```text
==> Exemple : Acheter lundi est peut-être une bonne idée ?
...
| Jour | Prix | Variation vs lundi (100 $) |
|---|---|---|
| Lundi | 100 $ | — (achat) |
| Mardi | 95 $ | **−5 $** |
| Mercredi | 1000 $ | **+900 $** |
...
> **Intuition humaine :** le mardi seul fait peur (perte de 5 $), mais à partir de mercredi le prix explose. Acheter lundi était en fait une **excellente** décision.

---

### 18.2 — Étape 1 — Les jugements TD(n) séparés

Chaque méthode TD(n) ne regarde **qu'un seul horizon** (un nombre fixe de pas).

```text
TD(0) : je regarde mardi seulement
95 - 100 = -5 $
=> Acheter lundi semble mauvais.
```

```text
TD(1) : je regarde jusqu'à mercredi
1000 - 100 = +900 $
=> Acheter lundi semble très bon.
```

```text
TD(2) : je regarde jusqu'à jeudi
980 - 100 = +880 $
=> Acheter lundi semble très bon.
```

```text
TD(3) : je regarde jusqu'à vendredi
970 - 100 = +870 $
=> Acheter lundi semble très bon.
```

| Méthode | Horizon | Différence calculée | Verdict |
|---|---|---|---|
| **TD(0)** | mardi (1 pas) | −5 $ | Mauvais |
| **TD(1)** | mercredi (2 pas) | +900 $ | Très bon |
| **TD(2)** | jeudi (3 pas) | +880 $ | Très bon |
| **TD(3)** | vendredi (4 pas) | +870 $ | Très bon |

> **Problème de TD(n) :** **on doit choisir un seul n**. Si on choisit TD(0), on conclut « mauvais ». Si on choisit TD(1), on conclut « très bon ». Le résultat dépend totalement du choix arbitraire de **n**.

---

### 18.3 — Étape 2 — TD(λ), c'est le mélange

> **TD(λ) ne choisit pas un horizon. Il fait une moyenne pondérée de TOUS les horizons**, avec des poids qui décroissent quand l'horizon s'éloigne.
...
|---|---|
| $G\_t^{\lambda}$ | Retour-λ — **moyenne pondérée** de tous les retours $G\_t^{(n)}$ |
| $G\_t^{(n)}$ | Retour à **n pas** (= ce que TD(n) calcule) |
| $\lambda \in [0, 1]$ | Coefficient de **mélange / vitesse d'oubli** |
| $\lambda^{n-1}$ | Plus l'horizon $n$ est lointain, plus son poids diminue |
| $(1-\lambda)$ | Facteur de normalisation pour que les poids somment à 1 |

---

### 18.4 — Calcul détaillé avec λ = 0,5

**Poids attribués à chaque horizon :**
...
> Les poids forment une **suite géométrique de raison λ = 0,5** : chaque horizon plus lointain pèse moitié moins que le précédent.
...
| Horizon | Poids λ^(n−1)·(1−λ) | Retour $G^{(n)}$ | Contribution |
|---|---|---|---|
| TD(0) (mardi) | 0,50 | −5 $ | −2,5 |
| TD(1) (mercredi) | 0,25 | +900 $ | +225 |
| TD(2) (jeudi) | 0,125 | +880 $ | +110 |
| TD(3) (vendredi) | 0,0625 | +870 $ | +54,37 |
| **Total ($G^{\lambda}$)** | ≈ 1 | — | **≈ +386,87 $** |

> **Conclusion chiffrée :** le retour-λ est **largement positif** → **acheter lundi était globalement une bonne décision**.

---

### 18.5 — Pourquoi c'est mieux que TD(0) seul

```mermaid
flowchart LR
    TD0["TD(0)<br/>Regarde mardi seulement<br/>−5 $"] --> CONCL0["Verdict : MAUVAIS"]
    TDL["TD(λ = 0.5)<br/>Mélange mardi + mercredi + jeudi + vendredi<br/>+386,87 $"] --> CONCLL["Verdict : TRÈS BON"]
    style TD0 fill:#dc2626,color:#fff
    style TDL fill:#16a34a,color:#fff
    style CONCL0 fill:#fee2e2
    style CONCLL fill:#dcfce7
```

| Méthode | Voix entendue dans la décision |
|---|---|
| **TD(0)** | « Mardi ça baisse, donc acheter lundi était **mauvais**. » |
| **TD(λ)** | « Attends ! Mardi était mauvais, mais mercredi, jeudi et vendredi sont **très bons**. Je mélange tout → acheter lundi était finalement une **bonne** décision. » |
...
> Les horizons **proches** comptent **plus**, les horizons **lointains** comptent **moins**.

Donc dans cet exemple :

```text
TD(0) juge trop vite avec mardi.
TD(λ) regarde mardi, mercredi, jeudi, vendredi ensemble.
```

Et comme mercredi, jeudi et vendredi sont très bons, **TD(λ) conclut que** :

```text
Acheter lundi était une bonne décision.
```

---

### 18.7 — Que se passe-t-il si on change λ ?

| Valeur de λ | Comportement | Verdict sur l'achat de lundi |
|---|---|---|
| **λ = 0** | Identique à **TD(0)** — ne regarde que mardi | « Mauvais » (−5 $) |
| **λ = 0,3** | Mardi domine encore, mais mercredi compte un peu | Légèrement positif |
| **λ = 0,5** | Compromis équilibré (calcul ci-dessus) | **+386,87 $ → bon** |
| **λ = 0,9** | Tous les horizons comptent presque autant | Très positif (proche de la moyenne) |
| **λ → 1** | Tend vers **Monte Carlo** (épisode complet) | Très positif (~+870 $) |

> **À retenir :** λ règle le **curseur** entre TD(0) (court terme uniquement) et Monte Carlo (épisode complet). Plus λ est grand, plus on tient compte du futur lointain.
...
```mermaid
flowchart TB
    Q["Acheter lundi à 100 $ : bonne décision ?"]
    Q --> TD0["TD(0)<br/>Mardi : 95 $<br/>Δ = −5 $<br/>MAUVAIS"]
    Q --> TD1["TD(1)<br/>Mercredi : 1000 $<br/>Δ = +900 $<br/>TRÈS BON"]
    Q --> TD2["TD(2)<br/>Jeudi : 980 $<br/>Δ = +880 $<br/>TRÈS BON"]
    Q --> TD3["TD(3)<br/>Vendredi : 970 $<br/>Δ = +870 $<br/>TRÈS BON"]
    TD0 --> MIX["TD(λ = 0,5)<br/>Mélange pondéré<br/>≈ +386,87 $<br/>BON"]
    TD1 --> MIX
    TD2 --> MIX
    TD3 --> MIX
    style TD0 fill:#fca5a5
    style TD1 fill:#86efac
    style TD2 fill:#86efac
    style TD3 fill:#86efac
    style MIX fill:#2563eb,color:#fff
    style Q fill:#fef3c7
```
